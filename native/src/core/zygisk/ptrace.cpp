...
#define CONT_OR_DIE \
    ...
    if (STOPPED_WITH(SIGSTOP, PTRACE_EVENT_STOP)) {
        // copy library to /dev
        std::string zygisklib = ZYGISK_LIB;
        zygisklib += std::to_string(pid);

        rm_rf(zygisklib.data());
        int src_fd = open("/proc/self/exe", O_RDONLY);
        int dest_fd = open(zygisklib.data(), O_RDWR | O_CREAT | O_TRUNC | O_CLOEXEC, 0644);
        struct stat st{};
        fstat(src_fd, &st);
        xsendfile(dest_fd, src_fd, nullptr, st.st_size);
        close(src_fd);
        close(dest_fd);

        chmod(zygisklib.data(), 0755);
        setfilecon(zygisklib.data(), "u:object_r:system_file:s0");

        ZLOGD("inject %s\n", zygisklib.data());
        bool is_injected = inject_on_main(pid, zygisklib.data());
